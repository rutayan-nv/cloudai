...
def test_validate_dse_env_params_rejects_non_dse(base_tr: TestRun) -> None:
    ...
    with pytest.raises(TestScenarioParsingError, match="declare env_params but are not agent-driven"):
...
def test_validate_dse_env_params_allows_live_rl_run() -> None:
    ...
    tdef.env_params = {"drop_rate": EnvParamSpec(values=[0.0, 0.1])}
...
def test_is_dse_or_live_rl(live_rl: bool, num_nodes: Any, expected: bool) -> None:
    assert _is_dse_or_live_rl(_routing_tr("tr", live_rl=live_rl, num_nodes=num_nodes)) is expected
